-- Phase 4A: atomically publish a submission and synchronize it to Hall master.
--
-- source_order is allocated from source-specific sequences. Sequence gaps are
-- acceptable; an allocated value is never reused or used to renumber rows.

create sequence public.hall_legacy_source_order_seq;
create sequence public.hall_submission_source_order_seq;

-- Verified immediately before this migration:
-- legacy_excel max(source_order) = 112
-- submission_user max(source_order) = 76
select setval('public.hall_legacy_source_order_seq'::regclass, 112, true);
select setval('public.hall_submission_source_order_seq'::regclass, 76, true);

revoke all on sequence public.hall_legacy_source_order_seq from public, anon, authenticated;
revoke all on sequence public.hall_submission_source_order_seq from public, anon, authenticated;
grant usage, select on sequence public.hall_legacy_source_order_seq to service_role;
grant usage, select on sequence public.hall_submission_source_order_seq to service_role;
...
begin
  ...
  -- Lock the submission so two administrators cannot make conflicting
  -- decisions or allocate two master rows from one submission.
  select *
  into submission_row
  ...
  -- Rejection keeps the existing Admin behavior but never enters Hall master.
  ...
  -- Lock an existing master row before deciding whether this is an update.
  -- This preserves provenance and source_order across retries and updates.
  select *
  into master_row
  from public.hall_cases_master
  where case_id = derived_case_id
  for update;
  ...
  if found then
    ...
    update public.hall_cases_master
    set nickname = submission_row.name,
        ...
        updated_at = now()
    where case_id = derived_case_id
    returning source_order into allocated_source_order;
  ...
  -- nextval is atomic and is intentionally inside the same database function
  -- as the insert. A failed transaction may leave a sequence gap, which is
  -- permitted by the stable-order contract.
...
-- This function is an internal server-side operation. Do not expose it through
-- the browser-facing anon/authenticated roles or through PUBLIC defaults.
